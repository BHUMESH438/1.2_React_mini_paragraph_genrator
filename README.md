...
enter the number from 1-8 it will genrate the paragraph according to the number given
...
<input type='number' name='amount' min='1' max='8' value={count} id='amount' onChange={e => setCount(e.target.value)} />
```
...
<article className='lorem-text'>
  {text.map(item => {
    return <p key={nanoid()}>{item}</p>; //child list should have unique id ;error will come if we dont pass the id we can also use mapindex but nanoid or uuid is preferable
  })}
</article>
```
